...
- Volume Docker MinIO chiamato `atixops_minio_data` (prefisso progetto docker-compose)
- Credenziali `PG_USERNAME` / `PG_DB_NAME`: lette dalle variabili d'ambiente se presenti, altrimenti dal file `.env` nella root del repo, altrimenti dai default `postgres` / `atix_backend`
...
```bash
# 1. Installa il cron job (crea da sé le directory sotto ~/backups/atixops)
./backup/install-cron.sh

# 2. Verifica che il cron sia attivo
crontab -l
```
